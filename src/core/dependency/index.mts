...
export async function resolve(specifier: string, context: ResolveContext, nextResolve: NextResolve): Promise<ResolutionResult> {
    const cacheKey = `${context.parentURL}::${specifier}`;
    if (resolutionCache.has(cacheKey)) return resolutionCache.get(cacheKey)!;

    const isBareSpecifier = !specifier.startsWith('.') && !specifier.startsWith('/') && !specifier.startsWith('file://') && !isBuiltin(specifier);
    ...
    if (isBareSpecifier && context.parentURL) {
        ...
        if (pluginMatch) {
            ...
            try {
                ...
                const result: ResolutionResult = { shortCircuit: true, url: pathToFileURL(absolutePath).href };
    ...
    try {
        ...
        if (error.code === 'ERR_MODULE_NOT_FOUND' && isBareSpecifier && context.parentURL) {
            ...
            if (pluginMatch) {
                throw new Error(`[Plugin Engine] '${pluginMatch[1]}' requested '${specifier}' but it was not found in its sandbox OR globally.`);
            }
        }
